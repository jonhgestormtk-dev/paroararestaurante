...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const activeOrders = allOrders.filter(o => {
      ...
      const matchesRes = restaurantFilter === 'all' || o.restaurantId === restaurantFilter;
      const isOngoing = o.status === 'Pendente' || o.status === 'Em Preparo' || o.status === 'Saiu para Entrega';
      ...
      <Card className="bg-white border-areia-escura rounded-[2rem] shadow-2xl overflow-hidden">
        <CardHeader className="p-8 border-b border-areia-escura/20 bg-areia-clara/10 flex flex-row items-center justify-between">
          <div className="space-y-1">
            ...
            <CardDescription className="font-subheadline italic text-xs">Exibindo apenas pedidos em andamento de hoje.</CardDescription>
          </div>
          <Badge className="bg-marrom-terra text-white uppercase text-[10px] font-black px-4 py-1">Tempo Real</Badge>
        </CardHeader>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      {isCritical && (
                        <Badge className="bg-rose-500 text-white animate-pulse text-[8px] font-black border-none uppercase px-2 py-0.5">
                          Atrasado
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-3 bg-white/40 p-4 rounded-2xl border border-areia-escura/30">
                    <div className="flex items-center gap-2">
                      <div className="flex flex-col">
                        <span className="text-[8px] font-black text-cinza-organico uppercase tracking-widest mb-1">Preparo</span>
